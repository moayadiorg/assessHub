...
async function main() {
  ...
  for (const u of [
    ...
    { email: 'reader@example.com', name: 'Reader User', role: 'reader' },
    { email: 'assessor@example.com', name: 'Assessor User', role: 'sa' },
    { email: 'moayad.ismail@gmail.com', name: 'Moayad Ismail', role: 'admin' },
  ]) {
    await pool.execute(
      ...
      [createId(), u.email, u.name, u.role]
    )
  }

  // Retrieve actual user IDs (may differ from generated ones if users already existed)
  const [userRows] = await pool.execute<any[]>(
    `SELECT id, email FROM User WHERE email IN (?, ?, ?, ?)`,
    ['admin@example.com', 'reader@example.com', 'assessor@example.com', 'moayad.ismail@gmail.com']
  )
  const userMap = new Map(userRows.map((r: any) => [r.email, r.id]))
  const moayadUserId = userMap.get('moayad.ismail@gmail.com')!
  const assessorUserId = userMap.get('assessor@example.com')!
  ...
  // Retrieve actual customer IDs
  const [custRows] = await pool.execute<any[]>(
    `SELECT id, name FROM Customer WHERE name IN (${customerNames.map(() => '?').join(',')})`,
    customerNames
  )
  for (const row of custRows) {
    customerIds[row.name] = row.id
  }
  console.log('✅ Customers created:', customerNames.length)
  ...
  async function findOrCreateType(
    ...
    const [existingRows] = await pool.execute<any[]>(
      `SELECT id FROM AssessmentType WHERE name = ?`,
      [name]
    )
    ...
    // Create the type and all nested data in a transaction
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO AssessmentType (id, name, description, version, iconColor, isActive, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, NOW(3), NOW(3))`,
        [typeId, name, description, version, iconColor, 1]
      )

      for (const category of categories) {
        const categoryId = createId()
        await connection.execute(
          `INSERT INTO Category (id, name, description, \`order\`, assessmentTypeId)
           VALUES (?, ?, ?, ?, ?)`,
          [categoryId, category.name, category.description, category.order, typeId]
        )

        for (const question of category.questions) {
          const questionId = createId()
          await connection.execute(
            `INSERT INTO Question (id, text, description, \`order\`, categoryId)
             VALUES (?, ?, ?, ?, ?)`,
            [questionId, question.text, question.description, question.order, categoryId]
          )

          for (const option of question.options) {
            const optionId = createId()
            await connection.execute(
              `INSERT INTO QuestionOption (id, score, label, description, questionId)
               VALUES (?, ?, ?, ?, ?)`,
              [optionId, option.score, option.label, option.description, questionId]
            )
          }
        }
      }
  ...
  async function findOrCreateAssessment(
    ...
    createdBy: string,
    status: string,
    responses?: Array<{ questionId: string; score: number; commentary: string | null }>
  ) {
    const [existingRows] = await pool.execute<any[]>(
      `SELECT id FROM Assessment WHERE name = ? AND customerName = ?`,
      [name, customerName]
    )
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO Assessment (id, name, customerName, customerId, assessmentTypeId, createdBy, status, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW(3), NOW(3))`,
        [assessmentId, name, customerName, customerId, assessmentTypeId, createdBy, status]
      )
        ...
        for (const response of responses) {
          ...
          await connection.execute(
            `INSERT INTO Response (id, assessmentId, questionId, score, commentary, createdAt, updatedAt)
             VALUES (?, ?, ?, ?, ?, NOW(3), NOW(3))`,
            [responseId, assessmentId, response.questionId, response.score, response.commentary]
          )
        }
      }
  ...
  const cloudType = await findOrCreateType(
    'Cloud Infrastructure Maturity',
    'Evaluate cloud infrastructure practices including IaC, CI/CD, monitoring, security, and cost management.',
    '2.0',
    '#3b82f6',
    [
      {
  ...
  const devopsType = await findOrCreateType(
    'DevOps Practices',
    'Assess DevOps culture, automation, and collaboration practices across the organization.',
    '1.0',
    '#8b5cf6',
    [
      {
  ...
  const dataType = await findOrCreateType(
    'Data Platform Maturity',
    'Evaluate data engineering, governance, and analytics capabilities.',
    '1.0',
    '#f59e0b',
    [
      {
  ...
  const iacType = await findOrCreateType(
    'HashiCorp IaC Capability Maturity',
    'Assess IaC capability maturity across automation workflows, quality standards, security & compliance, lifecycle management, and operational excellence. Aligned to HashiCorp\'s Infrastructure Lifecycle Management framework.',
    '1.0',
    '#7B42BC',
    [
      {
  ...
  await findOrCreateAssessment(
    'Cloud Infrastructure Review Q4 2025',
    'Acme Corp',
  ...
  await findOrCreateAssessment(
    'DevOps Assessment Q1 2026',
    'TechStart Inc',
    customerIds['TechStart Inc'],
    devopsType,
    assessorUserId,
    'in-progress',
    devopsQuestions.slice(0, 3).map((q, i) => ({
      questionId: q.id,
      score: [2, 2, 3][i],
      commentary: [
  ...
  await findOrCreateAssessment(
    ...
    'HealthFirst Solutions',
    customerIds['HealthFirst Solutions'],
    dataType,
    assessorUserId,
    'draft'
  )
  ...
  await findOrCreateAssessment(
    'DevOps Transformation Baseline',
    'RetailMax',
    customerIds['RetailMax'],
    devopsType,
    moayadUserId,
    'completed',
    devopsQuestions.map((q, i) => ({
      questionId: q.id,
      score: [1, 2, 2, 1, 2][i] || 1,
      commentary: [
  ...
  await findOrCreateAssessment(
    'Cloud Infrastructure Review Q1 2026',
    'Acme Corp',
    customerIds['Acme Corp'],
    cloudType,
    moayadUserId,
  ...
  await findOrCreateAssessment(
    'IaC Maturity Baseline Q1 2026',
    'Acme Corp',
    customerIds['Acme Corp'],
    iacType,
    moayadUserId,
    'completed',
    iacQuestions.map((q, i) => ({
      questionId: q.id,
  ...
  await findOrCreateAssessment(
    'IaC Security & Compliance Review',
    'GlobalBank',
    customerIds['GlobalBank'],
    iacType,
    moayadUserId,
  ...
  await findOrCreateAssessment(
    'IaC Compliance Readiness Assessment',
    'HealthFirst Solutions',
    customerIds['HealthFirst Solutions'],
    iacType,
    assessorUserId,
  ...
  try {
    await pool.execute(
      `DELETE FROM AssessmentType
       WHERE name = 'WARM Assessment'
       AND id NOT IN (SELECT DISTINCT assessmentTypeId FROM Category)`
    )
